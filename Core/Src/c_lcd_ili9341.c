...
static void SendDataDMA(uint8_t *data, uint16_t len)
{
    ...
    UINT dma_ret = tx_semaphore_get(&s_dmaSem, DMA_TIMEOUT_MS);
    if (dma_ret != TX_SUCCESS) {
        printf("[LCD] DMA timeout! len=%u ret=0x%02X\n", (unsigned)len, (unsigned)dma_ret);
    }
    CS_Deselect();
}
...
static void DisplayTaskEntry(ULONG argument)
{
    ...
    printf("[LCD] DisplayTask started\n");
    ...
    ILI9341_RegInit();
    printf("[LCD] RegInit done\n");

    /* Sleep Out */
    SendCmd(0x11);
    tx_thread_sleep(120);
    printf("[LCD] Sleep Out done\n");

    /* Display ON */
    SendCmd(0x29);
    tx_thread_sleep(20);
    printf("[LCD] Display ON done\n");
...
void LCD_Init(void)
{
    ...
    /* Display 태스크 생성 */
    UINT ret = tx_thread_create(&s_displayTask, "DisplayTask", DisplayTaskEntry, 0,
                     s_displayStack, sizeof(s_displayStack),
                     LCD_TASK_PRIO, LCD_TASK_PRIO,
                     TX_NO_TIME_SLICE, TX_AUTO_START);
    printf("[LCD] DisplayTask create ret=0x%02X\n", (unsigned)ret);
}
