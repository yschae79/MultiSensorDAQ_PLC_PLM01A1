...
### 8.7 이론상 안 될 것 같았는데 통신이 된 이유

> 실험자도 통신 불가를 예상했는데 실제로는 100% 성공했다.
> 아래 4가지 원인이 복합적으로 작용한 결과이다.

#### ① 실험실 환경 = 최선의 조건

| 실제 현장 | 이번 실험 |
|----------|----------|
| 수 m ~ 수십 m 배선 | 수십 cm (벤치 위) |
| SMPS 스위칭 노이즈 심함 | 노이즈 거의 없음 |
| 다수 부하 병렬 | 노드 2개만 |

#### ② PSU 커패시터는 이론치만큼 이상적이지 않음

$$X_{C_{PSU(ideal)}} \approx 1.4\text{ m}\Omega \quad \text{vs} \quad Z_{PSU(real)} \approx ESR + R_{trace} \approx 0.1\text{~}0.5\text{ }\Omega$$

전해 커패시터 ESR + 배선 저항으로 신호가 완전 단락되지 않고 **감쇠만 된 상태**로 전달됨.

#### ③ ST7580의 강인한 수신기

- **AGC(Automatic Gain Control)**: 신호 감쇠 시 수신 감도 자동 상향
- **BPSKCOD FEC**: rate-1/2 컨벌루셔널 코딩으로 ~5~6 dB 코딩 이득 확보
- 원래 **230V AC 상용 전원선** 에너버 노이즈 환경에서 동작하도록 설계된 칩

#### ④ RTT +48ms가 "간신히 동작"의 정량적 증거

```
신호 감쇠 → ST7580 AGC 최대 감도로 상향 → 복조 처리 시간 증가 → RTT +48ms
```

통신은 성공했지만 ST7580이 **최대 수신 감도로 간신히 버티는 상태**였음을 간접 증명한다.
정상 L_LINE 환경에서는 AGC 여유가 생겨 RTT +48ms가 해소될 것으로 예상한다.

#### 내부 L1/L2 교체만으로 불충분한 이유

PLM01A1 내부에는 이미 L1/L2 인덕터가 있지만 신호 정형용(~10~100 μH)이므로 PSU 격리에 필요한 mH급과는 20~200배 차이가 있다.

| 인덕터 | 용도 | 인덕턴스 | $X_L$ @ 110kHz |
|--------|------|----------|-----------------|
| 내부 L1/L2 | 신호 정형 | ~10~100 μH | ~7~69 Ω |
| 외부 L_LINE (목표) | PSU 격리 | 2.2 mH | ~1,520 Ω |

→ D4를 SM6T15CA로, C3/C6을 50V로 교체해도 **신호 흡수 문제는 해결되지 않는다**.
외부 L_LINE 추가만이 PSU 커패시터를 고주파 경로에서 분리시킬 수 있다.

> **결론**: 이번 실험은 *최상의 조건에서 한계까지 밀어붙였을 때 간신히 동작한* 것이다.
> 배선이 5 m만 되어도, 또는 SMPS 부하가 하나만 추가되어도 통신이 끊어졌을 가능성이 높다.
> **외부 L_LINE 커플링이 안정적 통신의 필수 조건**임을 이번 실험이 역설적으로 증명하였다.
